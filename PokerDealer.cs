	...
	public class PokerDealer : Mobile
	{
		...
		public bool Active
		{
			...
			set
			{
				...
				if (!value)
				{
					toRemove.AddRange(Game.Players.ToArray());
				}
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			else if (Game.IsPlayer(from as PlayerMobile))
			{
			...
			{
			    if (Game.Players.Any(player => player.Owner.NetState != null && @from.NetState != null &&
			                                           player.Owner.NetState.Address.Equals(@from.NetState.Address)))
			    {
		...
		{
			var toRemove = Game.Players.Where(player => player.Owner != null).ToList();
